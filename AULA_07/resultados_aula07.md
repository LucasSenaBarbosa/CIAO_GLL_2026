...
RESPOSTA: A mutação tem o papel de introduzir diversidade genética na população. Ela altera aleatoriamente alguns genes de um indivíduo, evitando que a população fique muito semelhante e ajudando o algoritmo a explorar novas soluções.
...
RESPOSTA: A penalização é fundamental porque impede que soluções que violam a restrição de capacidade sejam consideradas boas apenas por possuírem um valor total elevado.
...
RESPOSTA: Se c1 = 0, a componente cognitiva desaparece. Isso significa que as partículas deixam de considerar sua própria melhor posição histórica (pbest) para atualizar a velocidade.
...
RESPOSTA: Por que a evaporação do feromônio é necessária no algoritmo ACO?
